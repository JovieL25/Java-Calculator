...
public class CalculatorInterface {
	/**
	 * "history" is an array list which keeps track of user's math expression
	 * input and their results
	 */
	public static ArrayList<memory_node> history = new ArrayList<memory_node>(
			10);

	/**
	 * "rad" is a boolean value keep track of the output mode (Degree vs Radius)
	 */
		...
		while (mainMenuInput != 0) {
			...
			switch (mainMenuInput) {
				...
				case 5 :
					...
					while (keepGoing) {
						System.out.print(
								"Enter the values, use \",\" to separate: ");
						String x = sc.next();
						// replace with calculator function
				...
				case 9 :
					...
					while (keepGoing) { // user interface
						System.out
								.print("Write \"ANS1-10\" for using previous result"
						...
						if (userinput.contains("quit")) {
							...
							// System.out.println(userinput.contains("Show
							// History"));
							if (userinput.contains("Show History")) {
								...
								try {
									...
									result = Shunting_yard_algorithm
											.shunting_yard_algorithm_parse(
													userinput);

									if (memory_node.current_head < 10) {
										history.get(memory_node.current_head)
										...
										history.remove(0);
										history.add(new memory_node(userinput,
												result));
									}
									System.out.println(history
											.get(memory_node.current_head - 1)
											.getResult());
								} catch (Exception e) {
									e.printStackTrace();
								}
