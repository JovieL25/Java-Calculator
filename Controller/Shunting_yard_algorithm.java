...
public class Shunting_yard_algorithm {
	...
	/**
	 * "operators" array stores possible operators and their precedence level
	 * same as their index
	 */
	public static String[] operators = {"+-", "*/", "^"};
	private static int mad_input_size = -1;
	private static String[] TFs_labels = {"sinh", "ln", "exp", "mad", "sin",
			"cos", "tan"};

	/**
	 * isoperator function determines if a token is operators operators includes
	 * "+,-,*,/,(,),^"
	 */
	...
	/**
	 * isfunction function determines if a token is a function determines by
	 * finding any alphabets in the token
	 */
	...
	/**
	 * isnumber function determines if a token is a string anything that is not
	 * a string or having the described pattern is a number
	 */
	public static boolean isnumber(String a) {
		return !((isoperator(a)) || (isfunction(a))) || Pattern
				.compile("-?\\d+(\\.\\d+)?(e-?\\d+)?").matcher(a).find();
	}

	/**
	 * determine_precedence function is a helper function that help determine
	 * the precedence between two tokens if first token have higher precedence,
	 * the returned result will be larger 0
	 */
	public static int determine_precedence(String operator_head,
			String temp_token) throws Exception {
		...
		if (temp_token_precedence == -1 || operator_head_precedence == -1) {
			throw new Exception(
					"Error occured! Cannot determine the precedence");
		}
		return operator_head_precedence - temp_token_precedence;
	}

	/**
	 * shunting yard algorithm takes a string and identify token. This function
	 * will call RPN get the final result of the math expression.
	 */
	public static double shunting_yard_algorithm_parse(String expression)
			throws Exception {
		...
		// This while loop change all -( to -1*(
		// To reduce errors
		while (filtered_input_string.contains("-(") && !all_check) {
			string_index_begin = filtered_input_string.indexOf("-(",
					string_index_begin) - 1;

			if (string_index_begin == -1) {
				break;
			}

			if (Pattern.compile("[-+*/^]").matcher(filtered_input_string
					.substring(string_index_begin, string_index_begin + 1))
					.find()) {
				if (filtered_input_string
						.substring(string_index_begin, string_index_begin + 1)
						.equals("-")) {

					filtered_input_string = filtered_input_string.replace(
							filtered_input_string.substring(string_index_begin,
									string_index_begin + 1) + "-(",
							filtered_input_string.substring(string_index_begin,
									string_index_begin + 1) + "-1*(");
				} else {
					filtered_input_string = filtered_input_string.replace(
							filtered_input_string.substring(string_index_begin,
									string_index_begin + 1) + "-(",
					...
					string_index_end = filtered_input_string.indexOf(")",
							string_index_begin);
		...
		for (int i = 0; i < filtered_input_string.length(); i++) {
			...
			// If the previous character is number and current character is also
			// number
			...
			if (head.contains(",")) { // if current head contains a comma
				...
				temp_token = "";
					...
					|| !(isoperator(head) || isfunction(head))) {

				// If current char is a "." or it does not belong to operators
				// or functions
				...
				if (last_operator) {
					...
					if (temp_token.equals("")) { // if this is starting
													// character
						last_operator = false;
					} else if (isfunction(temp_token)
							|| temp_token.contains("(")) {
						...
						// I need to verify the precedence between new token and
						// top of operator stack
						// If Top of operator stack has more priority than
						// current token
						// Then pop the stack and pass the element to number
						// stack
						// Continue untill seeing a "(" on top of stack or
						// pushed if token have higher precedence
						int operation_index = operation_stack.size() - 1;
						String operator_head = "";

						if (operation_index > -1) {
							operator_head = operation_stack
									.get(operation_index);
						}

						while ((operation_index>-1)
								&& (isoperator(operator_head)
										&& isoperator(temp_token))
								&& !operator_head.contains("(")
								&& !temp_token.contains("^")) {
							if (determine_precedence(operator_head,
									temp_token) >= 0) {
				...
				/*
				 * If the current char is an operator There are several cases
				 * where:
				 * 
				 * case1: temp_token is a number, example 34+56 when head is
				 * "+", temp_token has "34". Simply push temp_token to
				 * number_stack
				 * 
				 * case2: temp_token has another operator, example 34+(5+6),
				 * when head is "(", temp_token has "+". This case push
				 * temp_token to operator_stack
				 * 
				 * extreme case: 34+-2, different from the above case, this case
				 * "-" is consider to be a part of number
				 * 
				 * 
				 */
				last_operator = true;
				if (head.equals("e") && !filtered_input_string
						.substring(i + 1, i + 2).equals("x")) {
					last_operator = false;
					last_number = true;
					temp_token += head;
				} else if (head.equals("-") && (i == 0 || (isoperator(
						filtered_input_string.substring(i - 1, i))
						&& !filtered_input_string.substring(i - 1, i)
								.equals(")")))) {

					// enter here if current char is "-"
					// I need to make sure if this is not part of an negative
					// number
					int operation_index = operation_stack.size() - 1;
					String operator_head = "";
					...
					// and do the routine described
					// above(checking precedence and pop to number stack)
					while (operation_index > -1
							&& (isoperator(operator_head)
									&& isoperator(temp_token))
							&& !operator_head.contains("(")
					...
					if (head.contains("(")) {
						...
						parenthesis = true;
						int operation_index = operation_stack.size() - 1;
						String operator_head = operation_stack
								.get(operation_index);
						while (!operator_head.contains("(")) {
							...
							if (operation_index == 0) {
								throw new Exception(
										"Error in the expression, parenthesis not matching");
							}
						...
						operation_stack.remove(operation_index);
						if (isfunction(
								operation_stack.get(--operation_index))) {
							number_stack.add(
									operation_stack.remove(operation_index));
						}
						temp_token = "";
					...
					if (isfunction(operation_stack.get(--operation_index))) {
						number_stack
								.add(operation_stack.remove(operation_index));
					}
					...
					if (isfunction(head) && !temp_token.equals("")
							&& !isfunction(temp_token)) {
						int operation_index = operation_stack.size() - 1;
						String operator_head = "";
						if (operation_index > -1) {
							operator_head = operation_stack
	...
	/**
	 * RPN function determine the result of the math expression that is in
	 * reverse polish notation. Shunting yard algorithm transfer a normal math
	...
	public static double RPN(ArrayList<String> stack) throws Exception {
			...
			if (stack.get(index).equals("")) {
				...
				switch (stack.get(index)) {
					...
					case "^" :
						...
						temp_token2 = processing_stack.pop();
						processing_stack
								.push(XtoN.Xton(temp_token2, temp_token1));
						break;
					case "sin" :
						temp_token1 = processing_stack.pop();
					...
					case "mad" :
						...
						for (int i = 1; i < mad_input_size; i++) {
							str_temp_token += Double
									.toString(processing_stack.pop()) + ",";
						}
						str_temp_token += Double
								.toString(processing_stack.pop());
						processing_stack.push(
								Mean_absolute_deviation.MAD(str_temp_token));
						break;
					case "sinh" :
						temp_token1 = processing_stack.pop();
						processing_stack.push(Sinh.sinh(temp_token1,
								!SimpleCalculatorInterface.rad_mode));
						break;
					case "exp" :
						temp_token1 = processing_stack.pop();
					...
					case "fact" :
						...
						if (BuiltInFunctions.isRational(temp_token1))
							throw new Exception(
									"Factorial input must be integer");
						processing_stack.push(
								BuiltInFunctions.factorial((int) temp_token1));
						break;
					case "sqrt" :
						temp_token1 = processing_stack.pop();
						processing_stack
								.push(BuiltInFunctions.sqrt(temp_token1));
						break;

					default :
						throw new Exception(
								"Unrecoginzed operators or functions");
				}
				stack.remove(index);
			}
		}
		...
		return Double.parseDouble(String.format("%.8f", result));
	}
	...
	private static void function_parenthesis_check(String a) throws Exception {
		...
		for (String c : TFs_labels) {
			...
			if (a.contains(c) 
					&& a.charAt(a.indexOf(c) + c.length()) != '('
					&& (!has_sinh && c.equals("sin"))) {
				throw new Exception("Function must followed by parenthesis");
			}

			if (a.contains("mad")) {
				Shunting_yard_algorithm.mad_input_size = a
						.substring(a.indexOf("mad") + 3,
								a.indexOf(")", a.indexOf("mad") + 3))
						.split(",").length;
			}
		}
	}
